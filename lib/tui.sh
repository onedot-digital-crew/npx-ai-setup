...
# ==============================================================================
# Sets REGEN_CLAUDE_MD, REGEN_AGENTS_MD, REGEN_CONTEXT,
# REGEN_AGENTS, REGEN_SKILLS globals.
# Returns 1 if the user selected nothing (skip).
ask_regen_parts() {
  _tui_init
  local options=("CLAUDE.md" "AGENTS.md" "Context" "Agents" "Skills")
  local descriptions=("Main project instructions" "Agent workflow guidelines" ".agents/context files" ".claude/agents files" ".claude/skills files")
  local count=5
  local selected=0
  ...
  [ "${checked[3]}" -eq 1 ] && REGEN_AGENTS="yes"
  [ "${checked[4]}" -eq 1 ] && REGEN_SKILLS="yes"

  if [ "$REGEN_CLAUDE_MD" = "no" ] && [ "$REGEN_AGENTS_MD" = "no" ] && [ "$REGEN_CONTEXT" = "no" ] && [ "$REGEN_AGENTS" = "no" ] && [ "$REGEN_SKILLS" = "no" ]; then
    echo ""
    return 1
  fi
  return 0
...
# ==============================================================================
# Sets UPD_HOOKS, UPD_SETTINGS, UPD_CLAUDE_MD, UPD_AGENTS_MD,
...
ask_update_parts() {
  _tui_init
  local options=("Hooks" "Settings" "CLAUDE.md" "AGENTS.md" "Agents" "Other")
  local base_descriptions=("Hooks and rules" ".claude/settings.json" "Root CLAUDE.md" "Root AGENTS.md" ".claude/agents" "Specs and misc files")
  local count=6
  local selected=0
  ...
  cat_changes=("${SCAN_HOOKS_CHANGED:-0}" "${SCAN_SETTINGS_CHANGED:-0}" "${SCAN_CLAUDE_MD_CHANGED:-0}" "${SCAN_AGENTS_MD_CHANGED:-0}" "${SCAN_AGENTS_CHANGED:-0}" "${SCAN_OTHER_CHANGED:-0}")
  cat_news=("${SCAN_HOOKS_NEW:-0}" "${SCAN_SETTINGS_NEW:-0}" "${SCAN_CLAUDE_MD_NEW:-0}" "${SCAN_AGENTS_MD_NEW:-0}" "${SCAN_AGENTS_NEW:-0}" "${SCAN_OTHER_NEW:-0}")
  ...
  [ "${checked[4]}" -eq 1 ] && UPD_AGENTS="yes"
  [ "${checked[5]}" -eq 1 ] && UPD_OTHER="yes"

  if [ "$UPD_HOOKS" = "no" ] && [ "$UPD_SETTINGS" = "no" ] && [ "$UPD_CLAUDE_MD" = "no" ] && [ "$UPD_AGENTS_MD" = "no" ] && [ "$UPD_AGENTS" = "no" ] && [ "$UPD_OTHER" = "no" ]; then
    echo ""
    return 1
  fi
  return 0
