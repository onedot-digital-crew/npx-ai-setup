...
| **Hooks**              | 9 hooks — protect-files, circuit-breaker, shellcheck-guard, graph-hints, post-edit-lint, context-freshness, update-check, task-completed-gate, precompact-guidance |
...
| `/review --spec 001` | Opus   | Review spec changes against acceptance criteria             |
...
| `/index`            | Sonnet | Stack-aware index: builds graph + manifest, refresh artifacts |
...
### Single-Source Sync (dev only)

`templates/` is the only source of truth. After editing any template, run:

```bash
bash bin/sync-local.sh           # write changes into .claude/, .github/, specs/
bash bin/sync-local.sh --check   # CI/quality-gate uses this — exit 1 on drift
bash bin/sync-local.sh --prune   # delete orphans (no template source)
```

Never edit `.claude/{agents,hooks,rules,scripts,docs,skills}` or `.github/` directly — the next sync will overwrite it. Marketplace/plugin/local-only skills (e.g. `agent-browser`, `gh-cli`, `orchestrate`, `bash-defensive-patterns`) are preserved.
...
The `graph-hints.sh` PreToolUse hook nudges Claude toward graph queries before expensive file reads or repeated Grep calls. It also injects forward/reverse import context for `Read`/`Edit` on `.ts/.tsx/.vue/.liquid` files (≤400 chars, jq cached per session).
