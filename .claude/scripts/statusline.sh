...
specs_count=""
if [ -d "$cwd/specs" ]; then
    count=$(find "$cwd/specs" -maxdepth 1 -name "*.md" 2>/dev/null | wc -l | tr -d ' ')
    [ "$count" -gt 0 ] && specs_count="$count"
fi

# ai-setup version
ai_setup_version=""
ai_setup_file="$cwd/.ai-setup.json"
if [ -f "$ai_setup_file" ]; then
    ai_setup_version=$(jq -r '.version // empty' "$ai_setup_file" 2>/dev/null)
fi
...
if [ -n "$specs_count" ]; then
    line1+="${sep}${dim}📋 ${reset}${white}${specs_count}${reset}"
fi
if [ -n "$ai_setup_version" ]; then
    line1+="${sep}${dim}v${ai_setup_version}${reset}"
fi
