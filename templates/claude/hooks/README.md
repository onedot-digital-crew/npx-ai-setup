...
- `post-compact-restore.sh` is registered for `PostCompact` first and `SessionStart(compact)` as fallback. `PostCompact` support is experimental and may not fire in all Claude builds.
