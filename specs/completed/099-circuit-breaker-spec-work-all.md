...
> **Spec ID**: 099 | **Created**: 2026-03-16 | **Status**: completed | **Branch**: —
...
## Steps
- [x] Step 1: In `templates/claude/hooks/circuit-breaker.sh`, after the existing spec-active check, add a second check — count files matching `Status.*in-progress` in `specs/*.md`; if count ≥ 2, raise WARN=25 and BLOCK=40
- [x] Step 2: Update `tests/smoke.sh` — add assertion that the circuit breaker hook contains the multi-spec detection string (e.g. `SPEC_COUNT` or the count comparison)

## Acceptance Criteria
- [x] Single in-progress spec: thresholds remain WARN=12 / BLOCK=20 (spec 094 behaviour preserved)
- [x] Two or more in-progress specs: thresholds raise to WARN=25 / BLOCK=40
- [x] No spec active: default thresholds WARN=5 / BLOCK=8
- [x] Smoke test passes for the new detection logic
