...
## Steps
- [ ] Step 1: Define cache file `.agents/.skill-cache.json` with schema: `{ "pkg_hash": "...", "stack_hash": "...", "cached_at": "ISO", "keywords": [...], "selected": [...] }`
- [ ] Step 2: At start of skill search in `lib/generate.sh` (~line 645), check if cache exists and both `pkg_hash` (package.json cksum) and `stack_hash` (.agents/context/STACK.md cksum) match — if both match, skip search and use cached `selected` list
- [ ] Step 3: After successful Claude ranking (~line 834), write cache file with both hashes, keywords, and selected skill IDs
...
- [ ] Changed `package.json` OR `STACK.md` triggers fresh search (hash mismatch invalidates cache)
